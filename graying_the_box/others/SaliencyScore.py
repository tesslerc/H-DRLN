...
axs.flat[1].imshow(Seaquest_states[Seaquestind], interpolation='none', cmap='gray')
my_cmap = plt.cm.get_cmap('hot')
my_cmap.set_bad('w')
my_cmap.set_under('w')
axs.flat[1].imshow(Seaquest_grads[Seaquestind], interpolation='none', cmap=my_cmap,vmin=.001,alpha=0.4)
axs.flat[1].set_title("Seaquest",fontsize=20)

axs.flat[0].imshow(Breakout_states[Breakoutind], interpolation='none', cmap='gray')
my_cmap = plt.cm.get_cmap('hot')
my_cmap.set_bad('w')
my_cmap.set_under('w')
axs.flat[0].imshow(Breakout_grads[Breakoutind], interpolation='none', cmap=my_cmap,vmin=.001,alpha=0.4)
axs.flat[0].set_title("Breakout",fontsize=20)

axs.flat[2].imshow(Pacman_states[Pacmanind], interpolation='none', cmap='gray')
my_cmap = plt.cm.get_cmap('hot')
my_cmap.set_bad('w')
my_cmap.set_under('w')
axs.flat[2].imshow(Pacman_grads[Pacmanind], interpolation='none', cmap=my_cmap,vmin=.001,alpha=0.4)
axs.flat[2].set_title("Pacman",fontsize=20)
plt.show()
